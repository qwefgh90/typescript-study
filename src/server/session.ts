namespace ts.server {
    ...
    export class Session<TMessage = string> implements EventSender {
        ...
        public event<T extends object>(body: T, eventName: string): void {
            const event = toEvent(eventName, body);
            this.logger.info("[CCW-Analysis-SendEvent]" + JSON.stringify(event)); // print event logs
        ...
        public output(info: any, cmdName: string, reqSeq?: number, errorMsg?: string): void {
            this.doOutput(undefined, info, cmdName, reqSeq!, /*success*/ !errorMsg, errorMsg); // TODO: GH#18217
        }

        private doOutput(request: protocol.Request | undefined, info: {} | undefined, cmdName: string, reqSeq: number, success: boolean, message?: string): void {
            ...
            this.logger.info("[CCW-Analysis-Request]" + JSON.stringify(request) + "  --->\n" + "[CCW-Analysis-Response]" + JSON.stringify(res)); // show relationship
            this.send(res);
        }
        ...
        */
        ...
        private reload(args: protocol.ReloadRequestArgs, reqSeq: number) {
            ...
            if (info) {
                ...
                if (info.reloadFromFile(tempFileName)) {
                    this.doOutput(undefined, /*info*/ undefined, CommandNames.Reload, reqSeq, /*success*/ true);
                }
            }
        }
        ...
        private handlers = new Map(getEntries<(request: protocol.Request) => HandlerResponse>({
            ...
            [CommandNames.Configure]: (request: protocol.ConfigureRequest) => {
                ...
                this.doOutput(request, /*info*/ undefined, CommandNames.Configure, request.seq, /*success*/ true);
                return this.notRequired();
            },
            ...
            [CommandNames.ConfigurePlugin]: (request: protocol.ConfigurePluginRequest) => {
                ...
                this.doOutput(request, /*info*/ undefined, CommandNames.ConfigurePlugin, request.seq, /*success*/ true);
                return this.notRequired();
            },
        ...
        public executeCommand(request: protocol.Request): HandlerResponse {
            ...
            if (handler) {
                ...
                this.doOutput(request, /*info*/ undefined, CommandNames.Unknown, request.seq, /*success*/ false, `Unrecognized JSON command: ${request.command}`);
                return { responseRequired: false };
            }
        }
        ...
        public onMessage(message: TMessage) {
            ...
            try {
                ...
                if (response) {
                    this.doOutput(request, response, request.command, request.seq, /*success*/ true);
                }
                else if (responseRequired) {
                    this.doOutput(request, /*info*/ undefined, request.command, request.seq, /*success*/ false, "No content available.");
                }
            }
            catch (err) {
                // Cancellation or an error may have left incomplete events on the tracing stack.
                ...
                if (err instanceof OperationCanceledException) {
                    ...
                    this.doOutput(request, { canceled: true }, request!.command, request!.seq, /*success*/ true);
                    return;
                }
